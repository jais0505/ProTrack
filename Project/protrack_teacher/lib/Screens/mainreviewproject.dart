...
class MainReviewProjectPage extends StatefulWidget {
  final int mid;
  final String type;
  const MainReviewProjectPage(
      {super.key, required this.mid, required this.type});

  @override
  State<MainReviewProjectPage> createState() => _MainReviewProjectPageState();
}

class _MainReviewProjectPageState extends State<MainReviewProjectPage> {
  ...
  Future<void> updateMark() async {
    try {
      ...
      await supabase.from('tbl_mainproject').update(
          {'mainproject_status': status}).eq('mainproject_id', widget.mid);
      print("Status:$status");
